...
const StudentGrading: React.FC = () => {
  ...
  const handleViewAttempt = async (attempt: ExamAttempt) => {
    try {
      ...
      // Load student data
      const studentData = await DatabaseService.getStudentById(
        attempt.student_id
      );
      ...
      const existingFeedback =
        typeof attempt.writing_feedback === "string"
          ? JSON.parse(attempt.writing_feedback)
          : attempt.writing_feedback || {};

      const existingScores =
        typeof attempt.scores === "string"
          ? JSON.parse(attempt.scores)
          : attempt.scores || {};

      // Check if already AI graded
      const isAIGraded = Object.values(existingFeedback).some(
        (feedback: any) => feedback?.assessedBy === "AI"
      );

      // Extract writing scores
      const writingTaskScores: Record<string, number> = {};
      if (examData?.sections) {
        const writingSection = examData.sections.find(
          (s) => s.type === "writing"
        );
        if (writingSection) {
          if (isAIGraded) {
      ...
      Object.entries(existingFeedback).forEach(
        ([questionId, feedbackData]: [string, any]) => {
  ...
  const isAIGraded = (attempt: ExamAttempt) => {
    try {
      ...
      return Object.values(feedbackData).some(
        (feedback: any) => feedback?.assessedBy === "AI"
      );
    } catch (error) {
      return false;
  ...
  const handleOverrideAIGrading = async () => {
    ...
    if (
      window.confirm(
        "This will override the AI grading. Are you sure you want to proceed?"
      )
    ) {
      try {
  ...
  const saveGrading = async () => {
    ...
    try {
      ...
      const existingScores =
        typeof selectedAttempt.scores === "string"
      ...
      const overallBand =
        ScoringSystem.calculateOverallBandScore(updatedScores);
  ...
  if (selectedAttempt && exam && student) {
    ...
    const attemptAnswers =
      typeof selectedAttempt.answers === "string"
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="font-semibold text-blue-900">
                      AI Pre-Graded Submission
                    </h3>
                    <p className="text-blue-800 text-sm">
                      This writing submission has been automatically graded by
                      AI. You can review the assessment, make adjustments, or
                      override it completely with your own grading.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {Object.entries(attemptScores).map(([section, score]) => (
                  ...
                  >
                    ...
                    <div
                      className={`text-2xl font-bold px-3 py-1 rounded-full ${getBandColor(
                        score as number
                      )}`}
                    >
                      {score}
                ...
                <div className="text-center p-4 bg-blue-50 rounded-lg">
                  ...
                  <p className="text-xs text-blue-600 mt-1">
                    {ScoringSystem.getPerformanceLevel(
                      selectedAttempt.overall_band
                    )}
                  </p>
                </div>
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Review and grade student exam submissions (AI-assisted)
            </p>
          </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {
                      examAttempts.filter((a) => a.status === "completed")
                        .length
                    }
                  </p>
                </div>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {
                      examAttempts.filter(
                        (a) => a.status === "graded" && isAIGraded(a)
                      ).length
                    }
                  </p>
                </div>
            ...
            {examAttempts.length > 0 ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-600">
                  Student exam submissions will appear here for grading.
                </p>
              </div>
            )}
          </div>
...
const ExamAttemptRow: React.FC<{
  ...
  return (
    <tr className="hover:bg-gray-50">
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        <span
          className={`inline-flex px-2 py-1 text-sm font-semibold rounded-full ${getBandColor(
