...
const ExamInterface: React.FC = () => {
    ...
    try {
      ...
      exam.sections.forEach((section) => {
        if (section.type === "reading") {
          const percentage = ScoringSystem.calculateReadingScore(
            section.questions,
            answers
          );
          ...
          const percentage = ScoringSystem.calculateListeningScore(
            section.questions,
            answers
          );
          const band = ScoringSystem.convertToBandScore(percentage);
      ...
      if (writingSection && writingSection.questions.length > 0) {
        try {
          const writingSubmissions = writingSection.questions.map(
            (question, index) => ({
          ...
          const aiAssessments = await GeminiAI.batchAssessWriting(
            writingSubmissions
          );

          const writingScores = Object.values(aiAssessments).map(
            (assessment) => assessment.bandScore
          );
          const writingBand =
            writingScores.reduce((sum, score) => sum + score, 0) /
            writingScores.length;

          scores.writing = Math.round(writingBand * 2) / 2;
          ...
          loadingToast.innerHTML =
            "✅ AI grading completed! Submitting exam...";
        } catch (aiError) {
          console.error("AI writing assessment failed:", aiError);
          loadingToast.innerHTML =
            "⚠️ AI grading failed. Submitting for manual review...";

          writingFeedback = Object.fromEntries(
            writingSection.questions.map((question) => [
              question.id,
              {
                feedback: `AI assessment failed: ${(aiError as Error).message}. Marked for manual review.`,
                assessedBy: "pending",
                assessedAt: new Date().toISOString(),
              },
            ])
      ...
      await DatabaseService.updateExamAttempt(attemptId, {
        ...
        status: scores.writing ? "graded" : "completed",
  ...
  const startExam = async () => {
    ...
    try {
      ...
      const result = await DatabaseService.createExamAttempt(
        exam.id,
        studentData.id
      );
          ...
          <div className="space-y-6">
            <div>
              ...
              <ul className="space-y-2 text-gray-700">
                ...
                <li>
                  • For listening sections, you can replay audio as needed
                </li>
                <li>
                  • Writing tasks will be submitted for teacher evaluation
                </li>
              </ul>
            </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {isMediaLoading && (
          <div className="mb-4 flex items-center justify-center p-4 bg-gray-50 rounded-lg border border-gray-200">
            <div className="flex items-center space-x-2 text-gray-600">
              ...
              <span>Loading question media...</span>
            </div>
          </div>
        )}
        <QuestionCard
